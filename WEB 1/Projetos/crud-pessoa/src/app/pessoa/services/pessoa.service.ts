...
httpOptions = {
  ...
  inserir(pessoa : Pessoa) : Observable<Pessoa | null> {
    return this.httpClient.post<Pessoa>(this.BASE_URL, JSON.stringify(pessoa), this.httpOptions).pipe(
      map((resp: HttpResponse<Pessoa>) => {
        if (resp.status !== 201) {
          return null;
        } else {
  ...
  atualizar(pessoa : Pessoa) : Observable<Pessoa | null> {
    return this.httpClient.put<Pessoa>(this.BASE_URL + "/" + pessoa.id, JSON.stringify(pessoa), this.httpOptions).pipe(
      map((resp: HttpResponse<Pessoa>) => {
        if (resp.status !== 200) {
          return null;
        } else {
