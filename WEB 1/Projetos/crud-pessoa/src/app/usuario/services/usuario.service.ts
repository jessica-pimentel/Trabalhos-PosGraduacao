...
export class UsuarioService {
  ...
  buscarPorId(id: number) : Observable<Usuario | null> {
    return this.httpClient.get<Usuario>(this.BASE_URL + "/" + id, this.httpOptions).pipe(
      map((resp: HttpResponse<Usuario>) => {
        if (resp.status != 200) {
          return null;
        }
  ...
  inserir(usuario: Usuario) : Observable<Usuario | null> {
    return this.httpClient.post<Usuario>(this.BASE_URL, JSON.stringify(usuario), this.httpOptions).pipe(
      map((resp: HttpResponse<Usuario>) => {
        if (resp.status != 201) {
          return null;
        }
  ...
  alterar(usuario: Usuario) : Observable<Usuario | null> {
    return this.httpClient.put<Usuario>(this.BASE_URL + "/" + usuario.id, JSON.stringify(usuario), this.httpOptions).pipe(
      map((resp: HttpResponse<Usuario>) => {
        if (resp.status != 200) {
          return null;
        }
